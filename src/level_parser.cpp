...
Emitter *createEmitter(const ofXml& data) {
    const ofPoint position = scaleToScreen(getVec2f(data.getChild("position")));
    const ofVec2f direction = getVec2f(data.getChild("direction"));

    const float maxVelocity = data.getChild("maxVelocity").getFloatValue();
    ...
    if (data.getChild("shape").getValue() == "POLYLINE")
        ...
        const ofVec2f size = scaleToScreen(getVec2f(data.getChild("size")));
...
Field *createField(const ofXml& data) {
    ...
    if (data.getChild("type").getValue() == "FORCE") {
        const ofVec2f force = getVec2f(data.getChild("force_or_color"));
